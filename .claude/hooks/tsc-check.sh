...
get_repo_for_file() {
    ...
    # Match apps/* or packages/* pattern
    if [[ "$relative_path" =~ ^(apps|packages)/([^/]+)/ ]]; then
        local workspace_dir="${BASH_REMATCH[1]}"
        local package_name="${BASH_REMATCH[2]}"
        echo "$workspace_dir/$package_name"
        return 0
    fi
    echo ""
    return 1
...
get_tsc_command() {
    ...
    # Use pnpm exec instead of npx for pnpm workspaces
    if [ -f "tsconfig.app.json" ]; then
        echo "pnpm exec tsc --project tsconfig.app.json --noEmit"
    elif [ -f "tsconfig.build.json" ]; then
        echo "pnpm exec tsc --project tsconfig.build.json --noEmit"
    elif [ -f "tsconfig.json" ]; then
        if grep -q '"references"' tsconfig.json 2>/dev/null; then
            if [ -f "tsconfig.app.json" ]; then
                echo "pnpm exec tsc --project tsconfig.app.json --noEmit"
            elif [ -f "tsconfig.src.json" ]; then
                echo "pnpm exec tsc --project tsconfig.src.json --noEmit"
            else
                echo "pnpm exec tsc --build --noEmit"
            fi
        else
