...
In this guide, we will look at how to register and consume webhooks to integrate your app with GLAPI. With webhooks, your app can know when something happens in Protocol, such as someone sending a message or adding a contact. {{ className: 'lead' }}
...
1.  **Verification:** GLAPI first verifies the webhook's signature (as described in the [Security](#security) section) to ensure it's legitimate.
...
*   **Automatic Syncing (Auto-Sync):**
    ...
    *   **How it works:** Each relevant event type (like `procore.requisition.created`, `netsuite.vendorbill.updated`, etc.) has an associated toggle in your project settings.
        *   If **enabled**, GLAPI will automatically try to process and sync the data related to the webhook event.
...
*Note: The exact event type names (`procore.resource.action`) may vary slightly. Refer to the Procore API documentation and your GLAPI project configuration for the specific event types available.*
