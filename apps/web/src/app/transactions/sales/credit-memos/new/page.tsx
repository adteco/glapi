...
const creditMemoFormSchema = z.object({
  customerId: z.string().min(1, "Customer is required"),
  transactionDate: z.string().min(1, "Credit memo date is required"),
  relatedInvoiceId: z.string().optional(),
  memo: z.string().max(1000, "Memo too long").optional(),
  lines: z.array(creditMemoLineSchema).min(1, "At least one line item is required"),
});

type CreditMemoFormValues = z.infer<typeof creditMemoFormSchema>;
...
export default function NewCreditMemoPage() {
  ...
  const handleSubmit = async (values: CreditMemoFormValues) => {
    // TODO: Connect to actual TRPC mutation when backend is ready
    console.log('Credit Memo data:', values);
    toast.success('Credit memo created successfully');
    router.push('/transactions/sales/credit-memos');
  };
  ...
  const totalGM = calculateGrossMargin(totals.amount, totals.cost);
  const totalGP = calculateGrossProfit(totals.amount, totals.cost);
  ...
  if (!orgId) {
    return <div className="container mx-auto py-10"><p>Please select an organization to create credit memos.</p></div>;
  }
    ...
    <div className="container mx-auto py-6 px-4 max-w-[1600px]">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          ...
          <p className="text-muted-foreground">Issue a credit to a customer</p>
        </div>
      </div>
        ...
        <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Select the customer for this credit memo</CardDescription>
            </CardHeader>
            <CardContent>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="Link to original invoice" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Set the date for this credit memo</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <FormField
          ...
          <Card className="bg-muted/30">
            <CardContent className="pt-6">
              <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
                ...
                <div className="text-center p-3 bg-background rounded-lg">
                  <p className="text-xs text-muted-foreground uppercase tracking-wide">GM</p>
                  <p className="text-lg font-semibold mt-1">{formatCurrency(totalGM)}</p>
                </div>
                <div className="text-center p-3 bg-background rounded-lg">
                  <p className="text-xs text-muted-foreground uppercase tracking-wide">GP%</p>
                  <p className="text-lg font-semibold mt-1">{formatPercent(totalGP)}</p>
                </div>
                <div className="text-center p-3 bg-primary/10 rounded-lg border-2 border-primary/20">
                  <p className="text-xs text-muted-foreground uppercase tracking-wide">Credit Total</p>
          ...
          <Card>
            <CardHeader>
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <CardDescription>Add items for this credit memo</CardDescription>
                </div>
                <Button
                  type="button"
                  variant="outline"
            ...
            <CardContent className="space-y-4">
              {fields.map((field, index) => {
                ...
                const cost = line?.cost || 0;
                const amount = calculateAmount(qty, price);
                const lineCost = calculateLineCost(qty, cost);
                const gm = calculateGrossMargin(amount, lineCost);
                const gp = calculateGrossProfit(amount, lineCost);
                ...
                return (
                  <div key={field.id} className="border rounded-lg p-4 space-y-4">
                    ...
                    {/* Row 3: Quantity, Unit Price, Amount, Unit Cost, GM, GP%, Line Total */}
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Add any notes for this credit memo</CardDescription>
            </CardHeader>
            <CardContent>
              <FormField
                control={form.control}
