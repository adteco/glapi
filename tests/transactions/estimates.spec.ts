...
import { uniqueId, randomString, formatDate, formatCurrency } from '../utils/test-helpers';
...
test.describe('Estimates', () => {
  ...
  test.describe('Page Load', () => {
    test('should load estimates list page', async ({ page }) => {
      await expect(page).toHaveURL(/\/transactions.*estimates/);
    });

    test('should display estimates table or empty state', async () => {
  ...
  test.describe('Search and Filter', () => {
    test('should search estimates by number', async () => {
      ...
      const searchTerm = firstRowText?.split(' ')[0] || 'EST';
  ...
  test.describe('Create Estimate', () => {
    test('should navigate to create estimate page', async ({ page }) => {
      await listPage.clickCreate();

      await expect(page).toHaveURL(/\/transactions.*estimates\/(new|create)/);
    });
    ...
    test('should calculate totals', async ({ page }) => {
      ...
      const totalDisplay = page.locator('[data-testid="total"], .estimate-total, :has-text("Total")');
    ...
    test('should display expiration date field', async ({ page }) => {
      ...
      const expirationField = page.locator('[name="expirationDate"], [data-testid="expiration-date"]');
      // Expiration date may or may not be visible depending on form design
    });
  });

  test.describe('View Estimate', () => {
    test('should navigate to estimate detail', async ({ page }) => {
      const rowCount = await listPage.getRowCount();
      if (rowCount === 0) {
        test.skip();
        return;
    ...
    test('should display estimate details', async ({ page }) => {
      ...
      const estimateNumber = page.locator('h1, h2, [data-testid="estimate-number"]');
      await expect(estimateNumber).toBeVisible();
  ...
  test.describe('Pagination', () => {
    test('should show pagination for many estimates', async () => {
      const rowCount = await listPage.getRowCount();
      if (rowCount >= 10) {
