...
export class ProjectReportingRepository extends BaseRepository {
  async getJobCostSummary(
    ...
    if (filters.projectIds?.length) {
      // Use inArray-style approach with raw SQL for UUID array
      const uuidList = filters.projectIds.map(id => `'${id}'::uuid`).join(', ');
      conditions.push(
        sql.raw(`p.id IN (${uuidList})`)
      );
    }
    ...
    return result.rows as unknown as JobCostSummaryRow[];
  }
}
