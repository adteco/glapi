...
export const entityRoles = pgTable(
  ...
  (table) => ({
    // Keep the assignment key aligned with the deployed schema. PostgreSQL
    // primary keys make every participating column NOT NULL, so including the
    // optional subsidiaryId here made global roles impossible and caused every
    // subsequent `drizzle-kit push` to try to reconcile the table again.
    primaryKey: primaryKey(table.entityId, table.roleId),
  }),
);
...
export const entitySubsidiaryAccess = pgTable(
  "entity_subsidiary_access",
  {
    entityId: uuid("entity_id")
      .notNull()
      .references(() => entities.id),
...
export const entitySubsidiaryAccessRelations = relations(
